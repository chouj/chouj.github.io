...
This plugin requires a valid and approved site key. You must signup and submit your blog to [Text link Ads](http://www.text-link-ads.com 'Text Link Ads')
...
3. If you downloaded the plugin from [www.text-link-ads.com](http://www.text-link-ads.com 'text link ads') as a logged in publisher you are all set, otherwise you will need to:
...
= 3.9.8 =
* show notice if no keys are set 

= 3.9.7 =
* compatible with inLinks. you can now run both inLinks and TLA on the same wordpress installation 
* no display ads on all pages
* you can now add multiple urls and xml keys via the settings
* JSON storage by default

= 3.5.1 =
* removed widget output if no ads found so no widget header will be shown
* removed any php warnings from code
* removed some class tags from widget display
